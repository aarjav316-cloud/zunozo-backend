...
export const setApprovedEventCache = async (events) => {
  await redisClient.set(APPROVED_EVENTS_KEY, JSON.stringify(events), {
    EX: CACHE_TTL,
  });
};
...
export const setEventCache = async (slug, event) => {
  await redisClient.set(`event:${slug}`, JSON.stringify(event), {
    EX: CACHE_TTL,
  });
};
